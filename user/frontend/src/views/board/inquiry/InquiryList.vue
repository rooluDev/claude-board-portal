      ...
      <v-table>
        ...
        <tbody>
          ...
          <tr v-for="board in boards" :key="board.boardId"
            ...
            <td>{{ board.boardId }}</td>
            <td>
              <v-icon v-if="board.isSecret" size="small" class="mr-1">mdi-lock</v-icon>
              {{ board.title }}
            </td>
            <td>{{ board.authorName }}</td>
            <td>{{ formatDate(board.createdAt) }}</td>
            <td>
              <v-chip :color="board.isAnswered ? 'success' : 'warning'" size="small">
  ...
  setup() {
    ...
    const fetchBoards = async () => {
      try {
        ...
        const response = await boardService.getList('inquiry', params)
    ...
    const goToView = (id) => {
      router.push({ name: 'InquiryView', params: { id } })
    }

    const goToWrite = () => {
      router.push({ name: 'InquiryWrite' })
    }
