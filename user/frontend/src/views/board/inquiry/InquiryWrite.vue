<template>
  <v-container>
    <h1 class="mb-4">{{ isEditMode ? '문의 수정' : '문의 작성' }}</h1>
...
export default {
  ...
  setup() {
    ...
    const fetchBoard = async () => {
      ...
      try {
        const response = await boardService.getDetail('inquiry', route.params.id)
        form.value = {
    ...
    const handleSubmit = async () => {
      ...
      formData.append('isSecret', form.value.isSecret)
      ...
      try {
        if (isEditMode.value) {
          await boardService.update('inquiry', route.params.id, formData)
          alert('수정되었습니다.')
          router.push({ name: 'InquiryView', params: { id: route.params.id } })
        } else {
          const response = await boardService.create('inquiry', formData)
          alert('등록되었습니다.')
          router.push({ name: 'InquiryView', params: { id: response.boardId } })
        }
      } catch (error) {
