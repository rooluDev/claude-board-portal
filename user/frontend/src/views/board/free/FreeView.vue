...
export default {
  ...
  setup() {
    ...
    const fetchBoard = async () => {
      try {
        // 조회수 증가
        await boardService.increaseView('free', route.params.id)

        // 상세 조회
        board.value = await boardService.getDetail('free', route.params.id)

        // 작성자 확인 (로그인 상태일 경우)
        if (isLoggedIn.value) {
          isAuthor.value = await boardService.checkAuthor('free', route.params.id)
        }
      } catch (error) {
    ...
    const goToList = () => {
      router.push({ name: 'FreeList' })
    }

    const goToEdit = () => {
      router.push({ name: 'FreeModify', params: { id: route.params.id } })
    }
    ...
    const handleDelete = async () => {
      ...
      try {
        await boardService.delete('free', route.params.id)
        alert('삭제되었습니다.')
        router.push({ name: 'FreeList' })
      } catch (error) {
